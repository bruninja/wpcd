...
function uf_epico_open_imagemodal(el){
	...
	frame.on('select',function(){
		...
		preview.css('background', 'url('+srcurl+') center center / '+response+' no-repeat').append('<span class="filechanger-btn uf_epico_media_select changer">Change Image</span>');
...
jQuery(function($){
	...
	if(typeof wp !== 'undefined'){
		...
		$('body').on('click','.image-field-uf-epico .remove', function() {
			...
			box.html('<span class="noselection uf_epico_media_select button">Select Image</span>').removeAttr('style');
		});
