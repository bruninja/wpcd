...
class Widget_Epico_Pages extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			'epico_pages-id',
			__( 'Epico Pages', 'uf-epico' ),
			array( 'description' => __( 'Create beautiful links for your special pages.', 'uf-epico' ) ),
			array( 'width'       => 495 )
		);
	...
	public function widget( $args, $instance ) {
		...
		echo $element->render_element( $instance, '', 'epico_pages' );
	...
	public function form( $instance ) {
		...
		if ( file_exists( self::get_path( dirname( __FILE__ ) ) . 'configs/fieldgroups-epico_pages.php' ) ) {
			include self::get_path( dirname( __FILE__ ) ) . 'configs/fieldgroups-epico_pages.php';
		} else {
			return;
		}
				...
				if ( is_active_sidebar( $sidebarid ) && $sidebarid != 'wp_inactive_widgets' && false === strpos( $sidebarid, 'orphaned_widgets' ) ) {
					foreach ( $widget_settings as $key => $settings ) {
						if ( in_array( $this->id_base . '-' . $key, $sidebar ) ) {
							...
							echo $element->render_element( $settings, '', 'epico_pages', true );
...
add_action( 'widgets_init', create_function( '', 'register_widget("Widget_Epico_Pages");' ) );
