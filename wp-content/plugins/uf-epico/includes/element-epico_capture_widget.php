...
$full_url = home_url( add_query_arg( array(), $wp->request ) ); // Obtem a URL atual

if ( 1 == $atts['innerpages'] ) {

	if ( is_front_page() ) {

		echo '<!--[if IE]><div' . ( isset( $custom_id ) ? ' id="cw-' . sanitize_html_class( $custom_id ) . '" ' : '' ) . ' class="capture-wrap uf-wrap ' . ( ( 2 == $atts['sidebar'] ) ? 'sb' : 'fw' ) . ' ie"><![endif]--><!--[if !IE]><!--><div' . ( isset( $custom_id ) ? ' id="cw-' . sanitize_html_class( $custom_id ) . '" ' : '' ) . ' class="capture-wrap uf-wrap ' . ( ( 2 == $atts['sidebar'] ) ? 'sb' : 'fw' ) . '"><!--<![endif]-->';
			...
			echo '<div class="capture uf">';
				...
				echo '<div class="capture-container uf-container ' . ( ( 1 == $atts['name_field'] ) ? 'uf-fields' : '' ) . '">';
					...
					if ( true == $atts['icon_upload'] ) {
						...
						echo '<p class="capture-icon uf-icon"><img src="' . esc_url( $atts['icon_upload'] ) . '" class="icon';
					...
					echo '<p itemprop="text" class="capture-intro uf-intro ' . ( ( 1 == $atts['disable_animation'] ) ? 'fadeinright' : '' ) . '">' . balanceTags( $atts['intro_p'], true ) . '<span class="uf-arrow"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 46.322 60"><polygon' . ( ( 1 == $atts['override'] ) ? ' fill="' . esc_attr( $atts['arrow_color'] ) . '"' : '' ) . ' points="33.831,36.839 23.161,0 12.491,36.839 0,36.839 23.161,60 46.322,36.839"/></svg></span></p>';
			...
			echo '<div class="capture capture-form uf ' . ( ( 1 == $atts['disable_animation'] ) ? 'animated' : '' ) . '">';
				...
				echo '<div class="capture-container uf-container ' . ( ( 1 == $atts['name_field'] ) ? 'uf-fields' : '' ) . '">';
					...
					if ( 1 == $atts['sidebar'] && ! empty( $atts['notice'] ) ) {

						echo '<p itemprop="text" class="capture-notice uf-notice ' . ( ( 1 == $atts['disable_animation'] ) ? 'fadeinleft' : '' ) . '">' . balanceTags( $atts['notice'], true ) . '</p>';
					}
					...
					if ( 2 == $atts['sidebar'] ) {
						...
						if ( 1 == $atts['close_btn'] ) {
							...
							echo '<div id="cw-' . sanitize_html_class( $custom_id ) . '-close" class="uf-close capture-close">
					...
					if ( 12 == $atts['email_service'] ) { // RD Station
						...
						echo '<form ' . ( isset( $custom_id ) ? 'id="' . sanitize_html_class( $custom_id ) . '" ' : '' ) . ( ( 1 == $atts['disable_animation'] ) ? 'class="fadeinright"' : '' ) . ' method="POST" action="https://www.rdstation.com.br/api/1.2/conversions" ' . ( ( 1 == $atts['new_window'] ) ? 'target="_blank" rel="noopener noreferrer"' : '' ) . '>';
							...
							<input type="hidden" name="c_utmz" id="c_utmz_' . $custom_id . '" value="" />
							
							<input type="hidden" name="traffic_source" id="traffic_source_' . $custom_id . '" value="" />';

							if ( 1 == $atts['tracking'] ) {

								echo '<input type="hidden" name="client_id" id="client_id_' . $custom_id . '" value="" />
					...
					if ( 19 == $atts['email_service'] ) { // Google Docs
						...
						echo '<form id="uf-input-form-' . esc_attr( $custom_id ) . '" ' . ( ( 1 == $atts['disable_animation'] ) ? 'class="fadeinright"' : '' ) . ' method="POST" action="' . ( isset( $atts['form_action'] ) ? esc_url( $atts['form_action'] ) : '' ) . '" accept-charset="UTF-8" target="no-target-' . esc_attr( $custom_id ) . '" onsubmit="submitted=true;">';
							...
							echo '<input id="uf-input-email-' . esc_attr( $custom_id ) . '" class="uf-email ' . ( ( 1 == $atts['disable_animation'] ) ? 'pulse' : '' ) . '" type="email" name="' . ( isset( $atts['email_field'] ) ? esc_attr( $atts['email_field'] ) : '' ) . '" placeholder="' . ( isset( $atts['placeholder'] ) ? ' ' . esc_attr( $atts['placeholder'] ) : '' ) . '">
		...
		echo '<!--[if IE]><div' . ( isset( $custom_id ) ? ' id="cw-' . sanitize_html_class( $custom_id ) . '" ' : '' ) . ' class="capture-wrap uf-wrap ip ' . ( ( 2 == $atts['sidebar'] ) ? 'sb' : 'fw' ) . ' ie"><![endif]--><!--[if !IE]><!--><div' . ( isset( $custom_id ) ? ' id="cw-' . sanitize_html_class( $custom_id ) . '" ' : '' ) . ' class="capture-wrap uf-wrap ip ' . ( ( 2 == $atts['sidebar'] ) ? 'sb' : 'fw' ) . '"><!--<![endif]-->';
			...
			echo '<div class="capture capture-inner uf uf-inner ' . ( ( 1 == $atts['disable_animation'] ) ? 'animated' : '' ) . '">';
				...
				if ( 1 == $atts['sidebar'] && ! empty( $atts['notice'] ) ) {
					...
					echo '<p itemprop="text" class="capture-notice uf-notice ' . ( ( 1 == $atts['disable_animation'] ) ? 'fadeinleft' : '' ) . '">' . $atts['title'] . '</p>';

				} else {

					echo '<p class="capture-title uf-title ' . ( ( 1 == $atts['disable_animation'] ) ? 'fadeinleft' : '' ) . '">' . $atts['title'] . '</p>';
				}

				if ( 1 == $atts['sidebar'] ) {

					if ( true == $atts['icon_upload'] && 2 == $atts['name_field'] ) {

						echo '<p class="capture-iconinner uf-iconinner"><img src="' . esc_url( $atts['icon_upload'] ) . '" class="icon';
						...
						if ( true == $atts['icon'] && 2 == $atts['name_field'] ) {

							echo '<p class="capture-iconinner uf-iconinner"><i class="' . esc_attr( $atts['icon'] );
				...
				if ( 2 == $atts['sidebar'] ) {
					...
					if ( 12 == $atts['email_service'] ) { // RD Station
						...
						echo '<form ' . ( isset( $custom_id ) ? 'id="' . sanitize_html_class( $custom_id ) . '" ' : '' ) . ( ( 1 == $atts['disable_animation'] ) ? 'class="fadeinright"' : '' ) . ' method="POST" action="https://www.rdstation.com.br/api/1.2/conversions" ' . ( ( 1 == $atts['new_window'] ) ? 'target="_blank" rel="noopener noreferrer"' : '' ) . '>';
							...
							<input type="hidden" name="c_utmz" id="c_utm_' . $custom_id . 'z_' . $custom_id . '" value="" />
...
} else { // Termina checagem da opcao inner pages
