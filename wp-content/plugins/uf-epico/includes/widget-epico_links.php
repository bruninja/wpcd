...
class Widget_Epico_Links extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			'epico_links-id',
			__( 'Epico Links', 'uf-epico' ),
			array( 'description' => __( 'Create beautiful links for your blog.', 'uf-epico' ) ),
			array( 'width'       => 495)
		);
	...
	public function widget( $args, $instance ) {
		...
		echo $element->render_element($instance, '', 'epico_links');
	...
	public function form( $instance ) {
		...
		if(file_exists(self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-epico_links.php')){
			include self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-epico_links.php';
		}else{
			return;
		}
		...
		for( $i=0; $i<$depth;$i++ ){
			...
			echo "<div class=\"form-table rowGroup groupitems\" id=\"groupitems\" ref=\"items\">\r\n";
				foreach($group['fields'] as $field=>$settings){
					...
					echo '<div class="uf-epico-field-row"><p><label class="uf-epico_widget_label" for="'.$id.'">'.$label.'</label></p>';
					...
					foreach($sidebar as $instance){
						if( false !== strpos( $instance, $this->id_base ) ){
							...
							if(!empty($widget_settings[$instance_id])){
								...
								echo $element->render_element($settings, '', 'epico_links', true);
...
add_action( 'widgets_init', create_function( '', 'register_widget("Widget_Epico_Links");' ) );
