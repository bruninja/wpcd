...
class Widget_Epico_Notice extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			'epico_notice-id',
			__( 'Epico Notice', 'uf-epico' ),
			array( 'description' => __( 'Create a cool notice at the top of your website.', 'uf-epico' ) ),
			array( 'width'       => 495 )
		);
	...
	public function widget( $args, $instance ) {
		...
		echo $element->render_element( $instance, '', 'epico_notice' );
	...
	public function form( $instance ) {
		...
		if ( file_exists( self::get_path( dirname( __FILE__ ) ) . 'configs/fieldgroups-epico_notice.php' ) ) {
			include self::get_path( dirname( __FILE__ ) ) . 'configs/fieldgroups-epico_notice.php';
		} else {
			return;
		}
	...
	public function widget_textdomain() {
		...
		load_plugin_textdomain( 'uf-epico', false, self::get_path( dirname( __FILE__ ) ) . '/lang/' );
				...
				if ( is_active_sidebar( $sidebarid ) && $sidebarid != 'wp_inactive_widgets' && false === strpos( $sidebarid, 'orphaned_widgets' ) ) {
					foreach ( $widget_settings as $key => $settings ) {
						if ( in_array( $this->id_base . '-' . $key, $sidebar ) ) {
							...
							echo $element->render_element( $settings, '', 'epico_notice', true );
							wp_enqueue_script( 'jquery', false, false, false, false );
							wp_enqueue_script( 'epico_notice-jquerycookieminjs', '//cdnjs.cloudflare.com/ajax/libs/jquery-cookie/1.4.1/jquery.cookie.min.js', false, false, true );
							wp_enqueue_script( 'epico_notice-epicoaviso', self::get_url( '/assets/js/uberaviso.js', dirname( __FILE__ ) ), array( 'jquery' ), false, true );
...
add_action( 'widgets_init', create_function( '', 'register_widget("Widget_Epico_Notice");' ) );
