...
class Widget_Epico_Author extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			'epico_author-id',
			__( 'Epico Author', 'uf-epico' ),
			array( 'description' => __( 'Custom author box widget.', 'uf-epico' ) ),
			array( 'width'       => 495 )
		);
	...
	public function widget( $args, $instance ) {
		...
		echo $element->render_element( $instance, '', 'epico_author' );
	...
	public function form( $instance ) {
		...
		if ( file_exists( self::get_path( dirname( __FILE__ ) ) . 'configs/fieldgroups-epico_author.php' ) ) {
			include self::get_path( dirname( __FILE__ ) ) . 'configs/fieldgroups-epico_author.php';
		} else {
			return;
		}
				...
				if ( is_active_sidebar( $sidebarid ) && $sidebarid != 'wp_inactive_widgets' && false === strpos( $sidebarid, 'orphaned_widgets' ) ) {
					foreach ( $widget_settings as $key => $settings ) {
						if ( in_array( $this->id_base . '-' . $key, $sidebar ) ) {
							...
							echo $element->render_element( $settings, '', 'epico_author', true );
...
add_action( 'widgets_init', create_function( '', 'register_widget("Widget_Epico_Author");' ) );
